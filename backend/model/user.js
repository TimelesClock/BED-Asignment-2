...
const user = {
    ...
    //enmdpoint 6
    get_films: (id) => {
        return query("SELECT film.film_id, film.title, category.name as category, film.rating, film.release_year, film.length as duration from film join film_category on film_category.film_id=film.film_id join category on category.category_id=film_category.category_id WHERE category.category_id =?;",
        [id]
        )
    },
    //endpoint 7
    ...
    addRent:(invID,body)=>{
        return query("INSERT INTO rental (rental_date,inventory_id,customer_id,staff_id) VALUES (?,?,?,?)", [new Date().toISOString().slice(0, 19).replace('T', ' '), invID, body.customer_id, body.staff_id])
    },

    addPayment:(ID,body)=>{
        return query("INSERT INTO payment (customer_id,staff_id,rental_id,amount,payment_date) VALUES (?,?,?,?,?)", [body.customer_id, body.staff_id, ID, body.amount, new Date().toISOString().slice(0, 19).replace('T', ' ')])
    },
